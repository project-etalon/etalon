"""
    ...
    It first launches OPEN AI server for different engines like vllm, lightllm, fastertransformers and sarathi-serve, and then runs -m metron.run_benchmark
"""
...
from metron.capacity_search.config.config import BenchmarkConfig, JobConfig
from metron.capacity_search.ray_utils import ReplicaResourceMapping, get_ip
...
def run(
    ...
    # Run the benchmark
    benchmark_command = f"python -m metron.run_benchmark {job_config.to_args()} {benchmark_config.to_args()}"
